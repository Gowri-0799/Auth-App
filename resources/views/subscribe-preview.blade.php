...
<form id="addonForm" action="{{ route('customer.subscribe', $newPlan->plan_code) }}" method="POST" style="display: none;">
    @csrf
    <input type="hidden" name="plan_code" value="{{ $newPlan->plan_code }}">
    <input type="hidden" name="zoho_cust_id" value="{{ $customer->zohocust_id ?? '' }}">
    <input type="hidden" name="zoho_cpid" value="{{  $partneruser->zoho_cpid ?? '' }}">
    <input type="hidden" name="plan_name" value="{{ $newPlan->plan_name ?? '' }}">
