...
<div class="modal fade" id="showAlertModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content bg-popup">
            <div class="modal-header border-0 bg-popup">
...
<script>
    document.addEventListener("DOMContentLoaded", function () {
        const showAlertModal = new bootstrap.Modal(document.getElementById("showAlertModal"));
        showAlertModal.show();
    });
</script>
@endpush
              ...
              <tr>
                ...
                <th class="align-middle position-relative">
                  <div>
                    ...
                    @endif
                    @else
                    @if ($plan->plan_price < $subscribedPlanPrice) <span class="text-muted"></span>
                      @else
                      @if ($subscriptions->isNotEmpty())

                      <form action="{{ route('upgrade.preview') }}" method="GET" style="display: inline;">
                        @csrf
                        <input type="hidden" name="plan_code" value="{{ $plan->plan_code }}">
